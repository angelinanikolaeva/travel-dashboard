...
for (let i = 0; i < tooltip.length; i++) {
  ...
  function showPopup(evt) {
    ...
    tooltip[i].style.left = iconPos.right - 310 + "px";
    tooltip[i].style.top = window.scrollY + iconPos.top - 400 + "px";
...
if (mapArr == null) {
  ...
  let countriesArray = spanArray.filter((el) =>
    el.includes('<span class="country-name">')
  );
  let chartCitiesSpan = spanArray.filter((el) =>
    el.includes('<span class="locality">')
  );
      ...
      datasets: [
        {
          label: "Continents",
          data: chartData,
          backgroundColor: ["rgba(248, 101, 73)", "rgba(237, 239, 239)"],
          borderWidth: 0,
          hoverOffset: 4,
        },
      ],
...
function createCard(data, index) {
  ...
  card.innerHTML = `<div class="inner-card">
                    ...
                    <p id="explore_countryname"><i class="fas fa-map-marker-alt"></i> ${data.countryname}</p>
                </div>`;
...
let searchBox;
let bounds;
let save = document.getElementById("map-button");
